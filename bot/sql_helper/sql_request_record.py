...
async def sql_get_request_record_by_tg(tg: int, page: int = 1, limit: int = 5):
    async with Session() as session:
        ...
        request_record = result.scalars().all()
...
async def sql_get_request_record_by_download_id(download_id: str):
    async with Session() as session:
        result = await session.execute(select(RequestRecord).filter(RequestRecord.download_id == download_id))
...
async def sql_get_request_record_by_transfer_state(transfer_state: str = None):
    async with Session() as session:
        result = await session.execute(select(RequestRecord).filter(RequestRecord.transfer_state == transfer_state))
