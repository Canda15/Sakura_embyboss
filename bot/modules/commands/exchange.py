...
async def rgs_code(_, msg, register_code):
    ...
    if embyid:
        ...
        async with Session() as session:
            # 使用 await session.execute + with_for_update 锁定并查询码
            result = await session.execute(select(Code).filter(Code.code == register_code).with_for_update())
            r = result.scalars().first()
            if not r:
                return await sendMessage(msg, "⛔ **你输入了一个错误de续期码，请确认好重试。**", timer=60)
            
            # 使用 session.execute(update...) 进行更新，只有当 used == None 时更新
            stmt = (
                update(Code)
                .where(Code.code == register_code)
                .where(Code.used.is_(None))
                .values(used=msg.from_user.id, usedtime=datetime.now())
            )
            re_res = await session.execute(stmt)
            await session.commit()
            
            tg1 = r.tg
            us1 = r.us
            used = r.used
            if re_res.rowcount == 0: 
