...
from bot import bot, _open, save_config, bot_photo, LOGGER, bot_name, admins, owner, config, registration_participants
...
async def open_stats(_, call):
    ...
    if stat:
        ...
        sur = all_user - tem
        part_count = len(registration_participants)
        text = f'🫧 管理员 {call.from_user.first_name} 已关闭 **自由注册**\n\n' \
               f'🎫 总注册限制 | {all_user}\n🎟️ 已注册人数 | {tem}\n' \
               f'🎭 剩余可注册 | **{sur}**\n🤖 bot使用人数 | {tg}\n' \
               f'👥 参与本次注册的总人数 | **{part_count}**'
        registration_participants.clear()
